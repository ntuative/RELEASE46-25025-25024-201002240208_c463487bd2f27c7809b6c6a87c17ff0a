package com.sulake.core.window.graphics
{
   ...
   public class WindowRenderer implements IWindowRenderer
   {
      ...
      public function WindowRenderer(param1:ISkinContainer)
      {
         ...
         var_569 = new Array();
         var_1460 = new Point();
         var_1461 = new Rectangle();
         var_293 = new Rectangle();
         var_294 = new Dictionary(true);
         var_668 = param1;
      ...
      {
         var _loc2_:WindowRendererItem = var_294[param1] as WindowRendererItem;
      ...
      public function render() : void
      {
         ...
         while(_loc4_ < _loc2_)
         {
            _loc1_ = var_569.pop() as WindowController;
            if(!_loc1_.disposed)
            {
               _loc3_ = getWindowRendererItem(_loc1_);
               var_293.x = _loc3_.dirty.x;
               var_293.y = _loc3_.dirty.y;
               var_293.width = _loc3_.dirty.width;
               var_293.height = _loc3_.dirty.height;
               renderWindowBranch(_loc1_,_loc3_,var_293);
            }
            _loc4_++;
         }
      }
      ...
      {
         param1.removeEventListener(WindowDisposeEvent.const_729,windowDisposedCallback);
         var _loc2_:WindowRendererItem = var_294[param1] as WindowRendererItem;
         if(_loc2_ != null)
         {
            _loc2_.dispose();
            delete var_294[param1];
         }
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            _disposed = true;
            var_569 = null;
            for(_loc1_ in var_294)
            {
               delete var_294[_loc1_];
            }
            var_294 = null;
      ...
      private function renderWindowBranch(param1:WindowController, param2:WindowRendererItem, param3:Rectangle) : void
      {
         ...
         if(param1.visible)
         {
            if(getDrawLocationAndClipRegion(param1,param3,var_1460,var_1461))
            {
               if(param2.render(param1,var_1460,var_1461))
               {
      ...
      public function registerRenderable(param1:IWindow) : void
      {
         ...
         if(_loc2_ == null)
         {
            _loc2_ = new WindowRendererItem(this,_drawBufferAllocator,var_668);
            var_294[param1] = _loc2_;
            param1.addEventListener(WindowDisposeEvent.const_729,windowDisposedCallback);
         }
      }
      ...
      public function invalidate(param1:IWindowContext, param2:Rectangle) : void
      {
         ...
         while(_loc4_-- > 0)
         {
            ...
            addToRenderQueue(_loc5_,_loc5_.rectangle,WindowRedrawFlag.const_69);
         }
      }
      ...
      public function addToRenderQueue(param1:IWindow, param2:Rectangle, param3:uint) : void
      {
         ...
         if(getWindowRendererItem(param1).invalidate(param1,param2,param3))
         {
            ...
            var_293.x = param2.x - param1.x;
            var_293.y = param2.y - param1.y;
            var_293.width = param2.width;
            var_293.height = param2.height;
            if(param1.testParamFlag(WindowParam.const_29))
            {
               while(param1.testParamFlag(WindowParam.const_29))
               {
                  ...
                  var_293.offset(param1.x,param1.y);
                  param1 = _loc4_;
               }
            }
            getWindowRendererItem(param1).invalidate(param1,var_293,WindowRedrawFlag.const_228);
            if(var_569.indexOf(param1) == -1)
            {
               var_569.push(param1);
            }
         }
