package com.sulake.habbo.ui
{
   ...
   public class RoomQueueWidgetHandler implements IRoomWidgetHandler
   {
      ...
      public function processEvent(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            case RoomSessionQueueEvent.const_409:
               ...
               {
                  case RoomSessionQueueEvent.const_832:
                     _loc3_ = "null";
                     break;
                  case RoomSessionQueueEvent.const_893:
                     _loc3_ = "null";
               }
               if(_loc3_ == null)
               {
               ...
               if(_loc5_.length > 1)
               {
                  if(_loc4_ && _loc2_.queueTypes.indexOf(RoomSessionQueueEvent.const_900) != -1)
                  {
                     _loc6_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_900) + 1;
                     _loc7_ = true;
                  }
                  else
                  {
                     _loc6_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_1303) + 1;
                  }
               }
               else
               {
      ...
      {
         var _loc1_:* = [];
         _loc1_.push(RoomWidgetRoomQueueMessage.const_535);
         _loc1_.push(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         _loc1_.push(RoomWidgetRoomQueueMessage.const_611);
         _loc1_.push(RoomWidgetRoomQueueMessage.const_524);
         return _loc1_;
      }
      ...
      public function processWidgetMessage(param1:RoomWidgetMessage) : RoomWidgetUpdateEvent
      {
         ...
         switch(param1.type)
         {
            case RoomWidgetRoomQueueMessage.const_535:
               _container.roomSession.quit();
               break;
            case RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_893);
               break;
            case RoomWidgetRoomQueueMessage.const_611:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_832);
               break;
            case RoomWidgetRoomQueueMessage.const_524:
               if(false)
               {
                  _container.catalog.openCatalogPage(CatalogPageName.const_234,true);
               }
         }
         return null;
      }
      ...
      {
         return [RoomSessionQueueEvent.const_409];
      }
