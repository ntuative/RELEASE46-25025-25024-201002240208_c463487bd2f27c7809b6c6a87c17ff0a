package com.sulake.habbo.widget.poll
{
   ...
   public class PollWidget extends RoomWidgetBase
   {
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetPollUpdateEvent.const_104,showPollOffer);
         param1.addEventListener(RoomWidgetPollUpdateEvent.const_63,showPollError);
         param1.addEventListener(RoomWidgetPollUpdateEvent.const_105,showPollContent);
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetPollUpdateEvent.const_104,showPollOffer);
         param1.removeEventListener(RoomWidgetPollUpdateEvent.const_63,showPollError);
         param1.removeEventListener(RoomWidgetPollUpdateEvent.const_105,showPollContent);
      }
      
      public function pollFinished(param1:int) : void
      {
         var _loc2_:PollSession = var_241.getValue(param1) as PollSession;
         if(_loc2_)
         {
            _loc2_.showThanks();
            _loc2_.dispose();
            var_241.remove(param1);
         }
      }
      ...
      private function showPollContent(param1:Event) : void
      {
         ...
         {
            _loc3_ = _loc2_.id;
            _loc4_ = var_241.getValue(_loc3_) as PollSession;
            if(_loc4_)
            {
      ...
      override public function dispose() : void
      {
         ...
         if(var_241)
         {
            ...
            while(_loc2_ < _loc1_)
            {
               _loc3_ = var_241.getWithIndex(0) as PollSession;
               if(_loc3_)
               {
                  _loc3_.dispose();
               }
