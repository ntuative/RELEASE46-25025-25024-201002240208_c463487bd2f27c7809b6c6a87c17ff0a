package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      private var var_2227:MarketplaceItemStats;
      ...
      private var var_182:MarketplaceConfirmationDialog;
      ...
      {
         if(!var_358 || !_catalog || !_catalog.getPurse())
         {
            return;
         }
         var _loc2_:MarketPlaceOfferData = var_358.getValue(param1) as MarketPlaceOfferData;
         if(!_loc2_)
         {
            return;
         }
         ...
         showConfirmation(const_1491,_loc2_);
      }
      ...
      {
         if(param1.furniCategoryId != var_2195 || param1.furniTypeId != var_2196)
         {
            return;
         }
         var_2227 = param1;
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         {
            var_358.remove(parser.requestedOfferId);
            _visualization.listUpdatedNotify();
            if(_windowManager != null)
            {
         ...
         {
            updateItem = var_358.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
            if(updateItem)
            {
               updateItem.offerId = parser.offerId;
               updateItem.price = parser.newPrice;
               --updateItem.offerCount;
               var_358.add(parser.offerId,updateItem);
            }
            var_358.remove(parser.requestedOfferId);
            showConfirmation(const_1578,updateItem);
            _visualization.listUpdatedNotify();
         }
      ...
      public function redeemSoldOffers() : void
      {
         var _loc2_:int = 0;
         var _loc3_:* = null;
         var _loc1_:Array = var_738.getKeys();
         for each(_loc2_ in _loc1_)
         {
            _loc3_ = var_738.getValue(_loc2_);
            if(_loc3_.status == MarketPlaceOfferState.const_430)
            {
               var_738.remove(_loc2_);
            }
         }
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         var_1847 = _loc3_.creditsWaiting;
         var_738 = new Map();
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            var_738.add(_loc4_.offerId,_loc5_);
         }
      ...
      private function showConfirmation(param1:int, param2:MarketPlaceOfferData) : void
      {
         if(!var_182)
         {
            var_182 = new MarketplaceConfirmationDialog(this,_catalog,_roomEngine);
         }
         var_182.showConfirmation(param1,param2);
      }
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            var_358.add(_loc4_.offerId,_loc6_);
         }
         if(_visualization == null)
         {
            return;
      ...
      {
         requestOffers(var_2225,var_2224,var_2140,var_2226);
      }
